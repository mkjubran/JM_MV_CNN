...
//printf("Jubran skipped P-MB (%2d, %2d), Number of parssed Bits = %d\n",currMB->mb_x, currMB->mb_y,se.len);
    }
...
//printf("Jubran ...Slice %2d, write MB %2d, NoCodedMB %3d, mode %2d, mb_y_coeff=%2d, mb_inter=%2d, mb_cbp=%2d, mb_delta_quant=%2d, mb_uv_coeff=%2d, mb_cb_coeff=%2d, mb_cr_coeff=%2d \n",currSlice->slice_nr, currMB->mbAddrX,p_Vid->NumberofCodedMacroBlocks, mbBits->mb_mode, mbBits->mb_y_coeff, mbBits->mb_inter, mbBits->mb_cbp, mbBits->mb_delta_quant, mbBits->mb_uv_coeff, mbBits->mb_cb_coeff, mbBits->mb_cr_coeff);
...
{
  Slice* currSlice = currMB->p_Slice;
  BitCounter Bits2count = currMB->bits;  //added by Jubran to count number of bits
  VideoParameters *p_Vid = currSlice->p_Vid;
  int           no_bits    = 0;
  SyntaxElement se;
  DataPartition *dataPart;
...
/* added by jubran to topen a file to count number of 4x4 macroblocks and identify their types
FILE *fptr = fopen("No4x4Macroblocks.dat", "ab+");
int buffNo=100;
int buffType=100;
fseek(fptr, 0, SEEK_SET); // make sure start from 0
while(!feof(fptr))
{
fscanf(fptr, "%d\n;", &buffNo); // read file *prefer using fscanf
}

if (currMB->mbAddrX != buffNo) 
{
printf("=======Macroblock: No=%d (%d,%d), Size=\n",currMB->mbAddrX, currMB->mb_x,currMB->mb_y); // for testing I'm using small integers
fprintf(fptr,"%d\n ",currMB->mbAddrX);
fscanf(fptr, "\n", &buffNo); // read file *prefer using fscanf
}
fclose(fptr);
...
// printf(" k=%3d, Level=%2d, Run=%2d, numcoeff=%2d, Numones=%2d, totzeros=%2d, numtrailingones=%2d, \n",k,level,run,numcoeff, numones,totzeros,numtrailingones); // added by jubran
