...
int encode_one_slice (VideoParameters *p_Vid, int SliceGroupId, int TotalCodedMBs)
{
 ...
 //  printf ("1encode_one_slice: mb %d,  slice %d,   bitbuf bytepos %d EOS %d\n", p_Vid->current_mb_nr, p_Vid->current_slice_nr, currSlice->partArr[0].bitstream->byte_pos, end_of_slice); //added byJubran

      write_macroblock (currMB, 1);
    }

//  printf ("2encode_one_slice: mb %d,  slice %d,   bitbuf bytepos %d EOS %d\n", p_Vid->current_mb_nr, p_Vid->current_slice_nr, currSlice->partArr[0].bitstream->byte_pos, end_of_slice); //added byJubran

    end_macroblock (currMB, &end_of_slice, &recode_macroblock);

    currMB->prev_recode_mb = recode_macroblock;
           
  //  printf ("3encode_one_slice: mb %d,  slice %d,   bitbuf bytepos %d EOS %d\n", p_Vid->current_mb_nr, p_Vid->current_slice_nr, currSlice->partArr[0].bitstream->byte_pos, end_of_slice);
